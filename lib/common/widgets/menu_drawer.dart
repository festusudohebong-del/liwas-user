...
class MenuDrawerState extends State<MenuDrawer> with SingleTickerProviderStateMixin {
  final List<Menu> _menuList = [
    ...
    Menu(icon: Images.orders, title: 'My orders'.tr, onTap: () {
      Get.back();
      Get.offAllNamed(RouteHelper.getOrderRoute());
    }),
    Menu(icon: Images.location, title: 'My address'.tr, onTap: () {
      Get.back();
    ...
    Menu(icon: Images.support, title: 'Help support'.tr, onTap: () {
      Get.back();
      Get.toNamed(RouteHelper.getSupportRoute());
    }),
    Menu(icon: Images.chat, title: 'Live chat'.tr, onTap: () {
      Get.back();
  ...
  void initState() {
    ...
    if(Get.find<SplashController>().configModel!.loyaltyPointStatus == 1) {
      _menuList.add(Menu(icon: Images.loyal, title: 'Loyalty points'.tr, onTap: () {
        Get.back();
        Get.toNamed(RouteHelper.getLoyaltyRoute());
      }));
    }
    if(Get.find<SplashController>().configModel!.refEarningStatus == 1) {
      _menuList.add(Menu(icon: Images.referCode, title: 'Refer and earn'.tr, onTap: () {
        Get.back();
    ...
    if(Get.find<SplashController>().configModel!.toggleDmRegistration!) {
      _menuList.add(Menu(
        icon: Images.deliveryManJoin, title: 'Join as a delivery man'.tr,onTap: (){
        Get.back();
    ...
    if(Get.find<SplashController>().configModel!.toggleStoreRegistration!) {
      _menuList.add(Menu(
        icon: Images.restaurantJoin, title: Get.find<SplashController>().configModel!.moduleConfig!.module!.showRestaurantText!
          ? 'Join as a restaurant'.tr : 'Join as a store'.tr,
        onTap: () {
          Get.back();
    ...
    _menuList.add(Menu(icon: Images.logOut, title: AuthHelper.isLoggedIn() ? 'Logout'.tr : 'Sign in'.tr, onTap: () {
      Get.back();
      if(AuthHelper.isLoggedIn()) {
        Get.dialog(ConfirmationDialog(icon: Images.support, description: 'Are you sure to logout'.tr, isLogOut: true, onYesPressed: () async {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Row( mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Menu'.tr, style: ralewayBold.copyWith(fontSize: 20 )),
                  ...
                  child: InkWell(
                    ...
                    child: Padding(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(child: Text(_menuList[index].title, style: ralewayMedium, overflow: TextOverflow.ellipsis, maxLines: 1)),
