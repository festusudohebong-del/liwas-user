  ...
  void _processFreeDeliveryCoupon(double deliveryCharge, double order) {
    if(deliveryCharge > 0) {
      if (_coupon!.minPurchase! <= order) {
        ...
        showCustomSnackBar('${'The minimum item purchase amount for this coupon is'.tr} '
            '${PriceConverter.convertPrice(_coupon!.minPurchase)} '
            '${'But you have'.tr} ${PriceConverter.convertPrice(order)}');
        _coupon = null;
        _discount = 0;
      }
    }else {
      showCustomSnackBar('Invalid code or'.tr);
    }
  }
