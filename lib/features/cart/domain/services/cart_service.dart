...
import 'package:liwas_user/features/cart/domain/models/online_cart_model.dart';
...
import 'package:liwas_user/features/item/domain/models/item_model.dart' as item_variation;
    ...
    if (isIncrement) {
      if(moduleStock && cartList[cartIndex].quantity! >= stock!) {
        ...
        if(quantity >= quantityLimit && quantityLimit != 0) {
          showCustomSnackBar('${'Maximum quantity limit'.tr} $quantityLimit');
        } else {
          quantity = quantity + 1;
        }
      } else {
