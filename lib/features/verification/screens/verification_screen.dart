...
import 'package:liwas_user/features/auth/domain/enum/centralize_login_enum.dart';
import 'package:liwas_user/features/auth/screens/new_user_setup_screen.dart';
import 'package:liwas_user/features/auth/widgets/sign_in/existing_user_bottom_sheet.dart';
...
import 'package:liwas_user/features/verification/domein/enum/verification_type_enum.dart';
import 'package:liwas_user/features/verification/domein/models/verification_data_model.dart';
...
class VerificationScreenState extends State<VerificationScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: isDesktop ? null : CustomAppBar(title: (_email != null && _email!.isNotEmpty) ? 'Email verification'.tr : 'Phone verification'.tr),
          ...
          child: GetBuilder<VerificationController>(builder: (verificationController) {
            return Column(children: [
              ...
              isDesktop ? Padding(
                ...
                child: Text(
                  'Otp verification'.tr, style: ralewayRegular,
                ),
              ) : const SizedBox(),
              ...
              Get.find<SplashController>().configModel!.demo! ? Text(
                'For demo purpose'.tr, style: ralewayMedium,
              ) : SizedBox(
                width: 250,
                child: Wrap(crossAxisAlignment: WrapCrossAlignment.center, children: [
                  RichText(text: TextSpan(children: [
                    TextSpan(text: 'We have a verification code'.tr, style: ralewayRegular.copyWith(color: Theme.of(context).hintColor)),
                    TextSpan(text: ' ${(_email != null && _email!.isNotEmpty) ? _email : _number}', style: ralewayMedium.copyWith(color: Theme.of(context).textTheme.bodyLarge!.color)),
              ...
              Padding(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text(
                    ...
                    style: ralewayRegular.copyWith(color: Theme.of(context).hintColor),
                  ),
                  TextButton(
                    ...
                    child: Text('${'Resent it'.tr}${_seconds > 0 ? ' (${_seconds}s)' : ''}', style: TextStyle(color: Theme.of(context).primaryColor),),
                  ),
                ]),
              ),
              const SizedBox(height: Dimensions.paddingSizeLarge),
      ...
      if(widget.loginType == CentralizeLoginType.otp.name) {
        Get.find<AuthController>().otpLogin(phone: _number!, otp: '', loginType: widget.loginType, verified: '').then((response) {
          if (response.isSuccess) {
            _startTimer();
            showCustomSnackBar('Resend code successful'.tr, isError: false);
          } else {
