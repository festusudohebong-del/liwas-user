import 'dart:async';
import 'package:liwas_user/common/widgets/custom_bottom_sheet_widget.dart';
...
import 'package:liwas_user/features/order/widgets/parcel_cancelation/cancellation_reason_bottom_sheet.dart';
import 'package:liwas_user/features/order/widgets/parcel_cancelation/slider_button_widget.dart';
...
import 'package:liwas_user/features/order/domain/models/order_details_model.dart';
...
import 'package:liwas_user/features/checkout/widgets/offline_success_dialog.dart';
import 'package:liwas_user/features/order/widgets/cancellation_dialogue_widget.dart';
import 'package:liwas_user/features/order/widgets/order_calcuation_widget.dart';
import 'package:liwas_user/features/order/widgets/order_info_widget.dart';
import 'package:liwas_user/features/review/screens/rate_review_screen.dart';
...
class OrderDetailsScreenState extends State<OrderDetailsScreen> {
      ...
      child: GetBuilder<OrderController>(builder: (orderController) {
        return Scaffold(
          appBar: isDesktop ? const WebMenuBar() : AppBar(
            title: Text(
              widget.orderModel?.orderType == 'parcel' ? 'Parcel details'.tr : 'Order details'.tr,
              style: ralewayMedium.copyWith(fontSize: Dimensions.fontSizeLarge, fontWeight: FontWeight.w600, color: Theme.of(context).textTheme.bodyLarge!.color),
            ),
            centerTitle: true,
            leading: IconButton(
          ...
          body: SafeArea(child: GetBuilder<OrderController>(builder: (orderController) {
            ...
            return orderController.orderDetails != null && order != null && orderController.trackModel != null ? Column(children: [
              ...
              isDesktop ? Container(
                ...
                child: Center(child: Text('Order details'.tr, style: ralewayMedium)),
              ) : const SizedBox(),
  ...
  Widget _buildActionButtonsRow({required bool showCancelButton, required bool showTrackDeliveryButton, required bool isDesktop, required OrderModel order,
    ...
    return Row(children: [
      if (showCancelButton)
        Expanded(
          child: CustomButton(
            ...
            buttonText: parcel ? 'Cancel delivery'.tr : 'Cancel order'.tr,
      ...
      if (showTrackDeliveryButton)
        Expanded(
          child: CustomButton(
            buttonText: parcel ? 'Track delivery'.tr : 'Track order'.tr,
    ...
    return SliderButton(
      ...
      action: () async {
        ...
        if (mounted && isSuccess) {
          showCustomSnackBar('Parcel returned successfully'.tr, isError: false);
        }
      },
    );
  }
  ...
  void _handleSwitchToCod(OrderController orderController, OrderModel order, bool parcel, double totalPrice) {
    Get.dialog(ConfirmationDialog(
      icon: Images.warning,
      description: 'Are you sure to switch'.tr,
      onYesPressed: () {
        ...
        if (canSwitchToCod) {
          ...
          if (Get.isDialogOpen!) Get.back();
             showCustomSnackBar('${'You cant order more then'.tr} ${PriceConverter.convertPrice(_maxCodOrderAmount)} ${'In cash on delivery'.tr}'
          );
        }
      },
    ));
