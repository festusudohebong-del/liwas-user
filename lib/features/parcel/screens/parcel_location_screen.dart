...
import 'package:liwas_user/features/parcel/widgets/parcel_view_widget.dart';
...
class _ParcelLocationScreenState extends State<ParcelLocationScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'Parcel location'.tr),
                      ...
                      TabBar(
                        ...
                        tabs: [
                          Container(
                            ...
                            child: Text(
                              'Sender info'.tr,
                              style: ralewayMedium.copyWith(fontSize: Dimensions.fontSizeLarge, color: parcelController.isSender ? Theme.of(context).cardColor : Theme.of(context).primaryColor),
                            ),
                          ),
                          ...
                          Container(
                            ...
                            child: Text(
                              'Receiver info'.tr,
                              style: ralewayMedium.copyWith(fontSize: Dimensions.fontSizeLarge, color: parcelController.isSender ? Theme.of(context).primaryColor : Theme.of(context).cardColor),
                            ),
                          ),
                        ],
                      ),
  ...
  Widget _bottomButton() {
    return GetBuilder<ParcelController>(builder: (parcelController) {
      return CustomButton(
        ...
        buttonText: parcelController.isSender ? 'Continue'.tr : 'Save and continue'.tr,
        onPressed: () async {
          if( _tabController!.index == 0 ) {
            ...
            if(parcelController.destinationAddress == null) {
              showCustomSnackBar('Select destination address'.tr);
            } else if(_receiverNameController.text.isEmpty){
              showCustomSnackBar('Enter receiver name'.tr);
            } else if(_receiverPhoneController.text.isEmpty){
              showCustomSnackBar('Enter receiver phone number'.tr);
  ...
  Future<void> _validateSender(ParcelController parcelController) async {
    ...
    if(parcelController.pickupAddress == null) {
      showCustomSnackBar('Select pickup address'.tr);
      _tabController!.animateTo(0);
    } else if(_senderNameController.text.isEmpty){
      showCustomSnackBar('Enter sender name'.tr);
      _tabController!.animateTo(0);
    } else if(_senderPhoneController.text.isEmpty){
      showCustomSnackBar('Enter sender phone number'.tr);
      _tabController!.animateTo(0);
    } else if (!phoneValid.isValid) {
      showCustomSnackBar('Invalid phone number'.tr);
      _tabController!.animateTo(0);
    }else if(AuthHelper.isGuestLoggedIn() && _guestSenderEmailController.text.isEmpty){
      showCustomSnackBar('Please enter sender email'.tr);
      _tabController!.animateTo(0);
