...
class _AddFundDialogueWidgetState extends State<AddFundDialogueWidget> {
      ...
      GetBuilder<WalletController>(builder: (walletController) {
        return Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('Add fund to wallet'.tr, style: ralewayBold.copyWith(fontSize: Dimensions.fontSizeLarge)),
            const SizedBox(height: Dimensions.paddingSizeSmall),

            Text('Add fund form secured digital payment gateways'.tr, style: ralewayRegular.copyWith(fontSize: Dimensions.fontSizeSmall), textAlign: TextAlign.center),
            const SizedBox(height: Dimensions.paddingSizeLarge),
            ...
            Flexible(
              child: RichText(
                text: TextSpan(children: [
                  TextSpan(text: 'Choose payment method'.tr, style: ralewayBold.copyWith(fontSize: Dimensions.fontSizeLarge, color: Theme.of(context).textTheme.bodyLarge!.color)),
                  const TextSpan(text: ' '),
                  TextSpan(
                    text: 'Faster and secure way to pay bill'.tr,
  ...
  void _onAddFundButtonClicked(WalletController walletController) {
    if(inputAmountController.text.isEmpty){
      showCustomSnackBar('Please provide transfer amount'.tr);
    }else if(inputAmountController.text == '0'){
      showCustomSnackBar('You can not add zero amount in wallet'.tr);
    }else if(walletController.digitalPaymentName == ''){
      showCustomSnackBar('Please select payment method'.tr);
    }else{
