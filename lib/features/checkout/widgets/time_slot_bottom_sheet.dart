...
import 'package:liwas_user/features/checkout/widgets/slot_widget.dart';
...
class _TimeSlotBottomSheetState extends State<TimeSlotBottomSheet> {
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Flexible(
                        child: ((checkoutController.selectedDateSlot == 0 && widget.todayClosed) || (checkoutController.selectedDateSlot == 1 && widget.tomorrowClosed))
                            ? Center(child: Text(widget.module!.showRestaurantText! ? 'Restaurant is closed'.tr : 'Store is closed'.tr))
                            ...
                            }) : Center(child: Text('No slot available'.tr)) : const Center(child: CircularProgressIndicator()),
                      ),
  ...
  Widget tabView({required BuildContext context, required String title, required bool isSelected, required Function() onTap}){
    return InkWell(
      ...
      child: Column(
        children: [
          Text(title, style: isSelected ? ralewayBold.copyWith(color: Theme.of(context).primaryColor) : ralewayMedium),
