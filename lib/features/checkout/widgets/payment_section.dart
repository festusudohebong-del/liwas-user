  ...
  Widget build(BuildContext context) {
    return Column(children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(storeId != null ? 'Payment method'.tr : 'Choose payment method'.tr, style: ralewayMedium),
      ...
      Container(
        ...
        child: storeId != null ? checkoutController.paymentMethodIndex == 0 ? Row(children: [
          ...
          Expanded(child: Text('Cash on delivery'.tr,
          ...
          child: Row(children: [
              ...
              child: Row(children: [
                Builder(
                  builder: (context) {
                    ...
                    return Text(
                      checkoutController.paymentMethodIndex == 0 ? '${'Cash on delivery'.tr} ${checkoutController.isPartialPay ? '(${'partial'.tr})' : ''}'
                          : checkoutController.paymentMethodIndex == 1 && !checkoutController.isPartialPay ? 'Wallet payment'.tr
                          : checkoutController.paymentMethodIndex == 2 ? '${'Digital payment'.tr} (${checkoutController.digitalPaymentName?.replaceAll('_', ' ').toTitleCase() ?? ''} - ${checkoutController.isPartialPay ? 'partial'.tr : ''})'
                          : checkoutController.paymentMethodIndex == 3 ? '${'Offline payment'.tr}(${checkoutController.offlineMethodList![checkoutController.selectedOfflineBankIndex].methodName} - ${checkoutController.isPartialPay ? 'partial'.tr : ''})'
                          : !ResponsiveHelper.isDesktop(context) ? 'Select payment method'.tr : 'Add payment method'.tr,
            ...
            checkoutController.paymentMethodIndex != -1 ? PriceConverter.convertAnimationPrice(
              ...
              textStyle: ralewayMedium.copyWith(fontSize: Dimensions.fontSizeLarge, color: Theme.of(context).primaryColor),
            ) : const SizedBox(),
