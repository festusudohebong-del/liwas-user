...
import 'package:liwas_user/features/checkout/widgets/checkout_screen_shimmer_view.dart';
import 'package:liwas_user/features/checkout/widgets/payment_method_bottom_sheet.dart';
import 'package:get/get.dart';
import 'package:liwas_user/features/checkout/widgets/bottom_section.dart';
import 'package:liwas_user/features/checkout/widgets/top_section.dart';
...
class CheckoutScreenState extends State<CheckoutScreen> {
            ...
            onPressed: checkoutController.acceptTerms
                ? () {
                    ...
                    if (isGuestLogIn &&
                      ...
                      showCustomSnackBar(
                          'Please setup your delivery address first'.tr);
                      ...
                      showCustomSnackBar(
                          'Please enter contact person number'.tr);
                    } else if (isGuestLogIn &&
                        checkoutController.orderType == 'take_away' &&
                        guestEmailController.text.isEmpty) {
                      showCustomSnackBar(
                          'Please enter contact person email'.tr);
                    } else if (isGuestLogIn &&
                      ...
                      showCustomSnackBar(
                          'You must upload prescription for this order'.tr);
                    } else if (!_isCashOnDeliveryActive! &&
                        !_isDigitalPaymentActive! &&
                        !_isWalletActive &&
                        !_isOfflinePaymentActive) {
                      showCustomSnackBar('No payment method is enabled'.tr);
                      ...
                      if (ResponsiveHelper.isDesktop(context)) {
                        if (_isCashOnDeliveryActive! ||
                          ...
                          showCustomSnackBar('No payment method found'.tr);
                        }
                      } else {
                        if (_isCashOnDeliveryActive! ||
                            _isDigitalPaymentActive! ||
                        ...
                        widget.storeId == null) {
                      showCustomSnackBar(
                          '${'Minimum order amount is'.tr} ${checkoutController.store!.minimumOrder}');
                      ...
                      showCustomSnackBar(Get.find<SplashController>()
                        ...
                        widget.storeId == null) {
                      showCustomSnackBar(
                          '${'You cant order more then'.tr} ${PriceConverter.convertPrice(maxCodOrderAmount)} ${'In cash on delivery'.tr}');
                    } else if (checkoutController.paymentMethodIndex != 0 &&
                        widget.storeId != null) {
                      showCustomSnackBar('Payment method is not available'.tr);
                    } else if (checkoutController.timeSlots == null ||
                        checkoutController.timeSlots!.isEmpty) {
                      if (checkoutController.store!.scheduleOrder!) {
                        showCustomSnackBar('Select a time'.tr);
                      } else {
                        showCustomSnackBar(Get.find<SplashController>()
                                .configModel!
                                .moduleConfig!
                      ...
                      showCustomSnackBar(
                          'One or more products are not available for this selected time'.tr);
                      ...
                      showCustomSnackBar('Delivery fee not set yet'.tr);
                        ...
                        checkoutController.pickedPrescriptions.isEmpty) {
                      showCustomSnackBar(
                          'Please upload your prescription images'.tr);
                    } else if (!checkoutController.acceptTerms) {
                      showCustomSnackBar(
                          'Please accept privacy policy trams conditions refund policy first'.tr);
                    } else {
  ...
  Future<void> showCashBackSnackBar() async {
    ...
    String text =
        '${'You will get'.tr} ${cashBackType == 'amount' ? PriceConverter.convertPrice(cashBackAmount) : '${cashBackAmount.toStringAsFixed(0)}%'} ${'Cash back after completing order'.tr}';
    if (cashBackAmount > 0) {
